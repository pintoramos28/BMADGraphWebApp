...
1. **Normalize** findings into a common format. Expected input formats:
   - Adversarial (Blind Hunter): markdown list of descriptions, ideally with P0/P1/P2/P3 priority in story-loop mode
   ...
   - Runtime Integration Auditor: JSON array or structured markdown with `location`, `runtime_surface`, `failure_mode`, `required_probe`, `potential_user_effect`, and optional `priority` fields
   - Acceptance Auditor: markdown list with title, optional priority, AC/constraint reference, and evidence
   ...
   Convert all to a unified list where each finding has:
    ...
    - `source` -- `blind`, `edge`, `runtime`, `acceptance`, or merged sources (e.g., `blind+edge`)
    - `title` -- one-line summary
    - `detail` -- full description
    - `location` -- file and line reference (if available)
    - `priority` -- P0, P1, P2, or P3 when provided or synthesized for story-loop mode
    - `priority_rationale` -- one-line reason in story-loop mode. If a reviewer lane omitted the rationale, synthesize the shortest conservative rationale from impact, acceptance-criteria risk, data-integrity risk, or runtime evidence before persisting the finding.
...
4. If story-loop mode is active, also assign a priority to every non-dismissed finding:
   ...
   Promote any questionable P3 to P2 when it could affect an acceptance criterion, security, data integrity, critical accessibility, or user-visible runtime correctness.

   Build these story-loop collections for Step 4:
   - `gating_findings`: all unresolved P0/P1/P2 findings with `action_state = "action_item"`.
   - `p3_findings`: P3 findings. Use `action_state = "action_item"` if any P0/P1/P2 finding exists in this review round; otherwise use `action_state = "deferred"` with `p3_rationale` and `disposition_reason`.
   - `decision_needed`: unresolved decisions with concrete options.
