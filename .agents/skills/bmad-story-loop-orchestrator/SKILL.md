---
name: bmad-story-loop-orchestrator
description: Automate BMAD story implementation and code-review loops by reading sprint status, launching fresh dev-story or code-review workers, allowing nested four-lane reviewer fan-out, polling workers with heartbeats, validating story/sprint status transitions, persisting a recoverable run ledger, enforcing strict JSON worker reports, looping until P0-P2 findings are resolved, and surfacing/defering P3 findings according to gating rules. Use when the user asks for a BMAD story loop, implementation-review loop, autonomous story orchestration, sprint story drain, or coordination of bmad-dev-story plus bmad-code-review without doing either directly.
---
...
- Let implementation workers use the configured implementation skill, defaulting to `bmad-dev-story`.
- Let review workers use the configured review skill, defaulting to `bmad-code-review`, and spawn the four reviewer lanes themselves.
...
Before delegation checks, story selection, or worker launch, resolve the requested inputs to concrete values and ask the user to confirm them. Do not begin the loop until the user explicitly confirms or edits the values.

Show a compact confirmation table with these fields and option descriptions:

| Input | Current value | Options / meaning |
| --- | --- | --- |
| `story_selector` | Provided story key/path, or `auto` | `auto` selects the next eligible story from sprint order; a story key targets that sprint entry; a path targets that exact story file. |
| `auto_advance` | `false` unless user requested drain/multiple stories | `false` processes only the selected/current story; `true` continues to the next eligible story after each story reaches `done`. |
| `max_stories` | `1` when `auto_advance=false`, otherwise `10`, unless provided | Safety cap for total stories processed in this run; only meaningful when `auto_advance=true` but still displayed for clarity. |
| `max_passes_per_story` | `10` unless provided | Maximum implementation+review cycles for a single story before stopping to prevent infinite loops. |
| `poll_interval_seconds` | `45` unless provided; never below `30` unless explicitly requested | Heartbeat/poll cadence for active workers; lower values are noisier and may waste tool/runtime quota. |
| `adapter_command` | Explicit path, discovered adapter path, or `direct BMAD fallback` | Adapter supplies repo-specific story discovery, review scope, and validation checks; direct fallback uses standard BMAD sprint/story conventions only. |

Then ask:

`Confirm these story-loop inputs? Reply "confirm" to proceed, or provide edits such as "story_selector=2-3-login", "auto_advance=true", "max_stories=3", "poll_interval_seconds=60", or "adapter_command=./.story-loop/adapter.sh".`

If the user edits any value, re-resolve dependent defaults and present the confirmation table again. Examples: if `auto_advance` changes from `false` to `true` and `max_stories` was not explicitly provided, update `max_stories` from `1` to `10`; if `adapter_command` changes, re-check adapter availability before proceeding. Continue this confirm/edit loop until the user explicitly confirms.
...
From `describe`, resolve:
- `implementation_skill` — default `bmad-dev-story` when omitted.
- `review_skill` — default `bmad-code-review` when omitted.

Pass these resolved skill names into the worker prompt templates. Do not hard-code worker skill names when the adapter supplies custom skills.
...
- worker/pass caps
- confirmed input values from the input confirmation gate
...
- story key
- current story status
- implementation skill from adapter/config, default `bmad-dev-story`
...
- story key
- current story status
- review skill from adapter/config, default `bmad-code-review`
- review scope from adapter/direct discovery
- repo-specific review checks
- instruction to run the configured review skill in orchestrated story-loop mode
